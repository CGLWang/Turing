$(function () {
  ...
  const fields = ['mqtt_us', 'mqtt_pw', 'mqtt_name']
  let mqtt_test_field = true
  for (let i in fields) {
    let a = $.cookie(fields[i]);
    if (a == undefined || a == '') {
      mqtt_test_field = false;
      break;
    }
  }
  if (mqtt_test_field) {
    mqtt_user_name = $.cookie(fields[0]);
    mqtt_psw = $.cookie(fields[1]);
    mqtt_client_id = $.cookie(fields[2]);
  } else {
    display_hint("cookies are broken")
  }
  ...
  function onMessageArrived(message) {
    if (/lily\/dev1\/bin\/(\d+)\/(\d+)\/(\d+)/.test(message.topic)) {
      ...
      if (segn == seg) {
        // pic there
        var blob = new Blob([Uint8Array.from(bin_stack)], { type: "image/png" });
  ...
  if (d_namesx.length > 0) {
    ...
    $("#devices_avaliable > tr").each(function (i, self) {
      $(this).click(function () {
        ...
        $("#devices_avaliable > tr")[using_dev_index].classList.remove('bg-light')
        using_dev_index = i;
  ...
  setInterval(query_auth, 60000); //60s
  if (!in_mobile)
    $("#chat_log").niceScroll();
